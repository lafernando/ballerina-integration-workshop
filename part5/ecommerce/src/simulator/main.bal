...
public function doSession(int accountId, boolean doError1, boolean doError2) returns @tainted error? {
    ...
    _ = check adminClient->post("/cartitems/" + accountId.toString(), <@untainted> check item1.cloneWithType(json));
    _ = check adminClient->post("/cartitems/" + accountId.toString(), <@untainted> check item2.cloneWithType(json));
    if doError1 {
